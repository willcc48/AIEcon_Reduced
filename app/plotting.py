...
def plot_map(maps, locs, ax=None, cmap_order=None):
    ...
    max_health = {"Wood": 1, "Stone": 1, "Gold": 1, "House": 1}
    n_agents = len(locs)
...
def breakdown(log, remap_key=None):
    ...
    if trading_active:
        ...
        incomes = {
            ...
            "Sell Gold": [
                sum([t["income"] for t in c_trades["Gold"] if t["seller"] == aidx[i]])
                for i in range(n)
            ],
            "Buy Gold": [
                sum([-t["price"] for t in c_trades["Gold"] if t["buyer"] == aidx[i]])
                for i in range(n)
            ],
            "Build": [
